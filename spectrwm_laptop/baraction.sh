...
CPUTEMP() {
  CPUTEMP="$(sensors | grep Package | awk '{print $4}' | sed 's/"//g')"
  echo "$CPUTEMP"
}
...
#	tmp="$(grep temp_C ~/.config/weather.txt | awk '{print $2}' | sed 's/"//g' | sed 's/,/ C/g')"
       temp="$(cat ~/.config/weather_temp.txt)" 	
       echo " $temp"
}
## BATTERY
bat() {
...
#echo " | +@fg=6; $(cpu)    $(CPUTEMP) +@fg=0; | +@fg=2; $(mem) +@fg=0; | +@fg=3; $(hdd)  $(hdd1) +@fg=0; | +@fg=1; $(network)  $(netspeed) +@fg=0; | +@fg=7; $(upgrades) +@fg=0; | $(weather) $(temp) | +@fg=4; $(vol) +@fg=0; | $(bat) | +@fg=6; $(dato) +@fg=0; | "
	
echo " | +@fg=6; +@fn=1; +@fn=0; $(cpu)    $(CPUTEMP) +@fg=0;| +@fg=4; +@fn=1; +@fn=0; $(mem) +@fg=0; | +@fg=1; +@fn=1; +@fn=0; $(hdd)   $(hdd1) +@fg=0; | +@fg=3; +@fn=1; +@fn=0; $(network)  $(netspeed) +@fg=0; | +@fg=7; +@fn=1; +@fn=0; $(upgrades) +@fg=0; | +@fg=2; +@fn=1; +@fn=0; $(weather) $(temp) +@fg=0; | +@fg=5; +@fn=1; +@fn=0; $(vol) +@fg=0; | +@fg=7; +@fn=1; +@fn=0; $(bat) +@fg=0; | +@fn=1; +@fn=0; $(dato) |"
